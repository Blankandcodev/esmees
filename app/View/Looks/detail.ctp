			...
			<div class="slideshow">
				<ul>
					<?php foreach($memberImages as $mimage){?>
						<li>
							<a href="<?php echo $this->Html->url(array('controller'=>'Looks', 'action'=>'memberdetails', $_look['product_id']),true) ?>"><?php echo $this->Html->image('Looks/small/'.$mimage['Look']['image']);?></a>
						</li>
					<?php } ?>
				</ul>
			</div>
